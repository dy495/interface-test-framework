...
public class AppVoiceCase extends TestCaseCommon implements TestCaseStd {
    ...
    //有问题
    @Test(description = "各话术环节分数相加/5=APP接待详情中的部门平均分", enabled = false)
    ...
    public void department_data_22() {
        ...
        try {
            ...
            int mathResult = CommonUtil.getCeilIntRatio(scoreSum, 5);
            CommonUtil.valueView(totalAverageScore, mathResult);
            Preconditions.checkArgument(scoreSum == mathResult || scoreSum >= mathResult + 1 || scoreSum <= mathResult - 1, "【部门总平均分趋势】平均分：" + scoreSum + " 【我的能力模型】各话术环节平均分之和/5：" + mathResult);
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {

...
public class MarketingManageOnline extends TestCaseCommon implements TestCaseStd {
    ...
    public void voucherManage_data_2() {
        ...
        try {
            ...
            //创建卡券
            String voucherName = util.createVoucher(1000);
            //作废此卡券
    ...
    public void voucherManage_data_3() {
        ...
        try {
            ...
            long newSurplusInventory = voucher.getSurplusInventory() == null ? 0 : voucher.getSurplusInventory();
            Long newAdditionalInventory = voucher.getAdditionalInventory();
    ...
    public void voucherManage_data_5() {
        ...
        try {
            ...
            //审批后数据
            VoucherPage voucher = util.getVoucherInfo(voucherName);
            Long newSurplusInventory = voucher.getSurplusInventory();
            Long newAdditionalInventory = voucher.getAdditionalInventory();
            CommonUtil.valueView(surplusInventory, newSurplusInventory, additionalInventory, newAdditionalInventory);
            if (surplusInventory != 0) {
                Preconditions.checkArgument(newSurplusInventory == surplusInventory,
                ...
                for (int j = 0; j < array.size(); j++) {
                    if (array.getJSONObject(j).getString("invalid_status_name").equals(EnumVoucherStatus.UNSENT.getName())) {
                        ...
                        Preconditions.checkArgument(cumulativeDelivery == 0,
                                voucherName + "累计发出数量：" + CommonUtil.result(0, cumulativeDelivery));
                    }
                }
            }
        } catch (Exception | AssertionError e) {
                ...
                array.forEach(e -> {
                    ...
                    if (jsonObject.getString("invalid_status_name").equals(EnumVoucherStatus.SENT.getName())) {
                        ...
                        Preconditions.checkArgument(cumulativeDelivery > 0, voucherName + "累计发出数量：" +
                                CommonUtil.result(">0", cumulativeDelivery));
                        CommonUtil.logger(voucherName);
                    }
                });
            }
    ...
    public void voucherManage_data_10() {
        ...
        try {
            ...
            IScene scene1 = VoucherVerificationScene.builder().id(String.valueOf(id)).verificationCode(code).build();
            jc.invokeApi(scene1);
            user.login(administrator);
            //核销后数据
            ...
            Preconditions.checkArgument(newCumulativeUse == cumulativeUse + 1,
                    voucherName + "被核销后" + CommonUtil.result(cumulativeUse + 1, newCumulativeUse));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void voucherManage_data_11() {
        ...
        try {
            ...
            IScene temporaryScene = PurchaseTemporaryPackageScene.builder().customerPhone(marketing.getPhone())
            ...
            Preconditions.checkArgument(newCumulativeDelivery == cumulativeDelivery + 1,
                    voucherName + "累计发出数：" + CommonUtil.result(cumulativeDelivery + 1, newCumulativeDelivery));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
            ...
            for (int i = 1; i < s; i++) {
                ...
                for (int j = 0; j < array.size(); j++) {
                    ...
                    if (Long.parseLong(DateTimeUtil.dateToStamp(createTime, format)) > time) {
                        ...
                        IScene scene = VerificationRecordScene.builder().voucherName(voucherName).build();
                        //核销数量
        ...
        try {
            ...
            for (int i = 1; i < s; i++) {
                ...
                for (int j = 0; j < array.size(); j++) {
                    ...
                    int numSum = 0;
                    //卡券审核页
                    ApplyPageScene.ApplyPageSceneBuilder applyBuilder = ApplyPageScene.builder().status("AGREE").name(voucherName);
    ...
    public void voucherManage_data_17() {
        ...
        try {
            ...
            //创建一种卡券
            util.createVoucher(1000);
            //创建卡券后卡券审核页列表数
            Long newTotal = jc.invokeApi(builder.build()).getLong("total");
            Preconditions.checkArgument(newTotal == total + 1,
                    "【卡券审核】列表数：" + CommonUtil.result(total + 1, newTotal));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void voucherManage_data_18() {
        ...
        try {
            //增发卡券前卡券审核页列表数
            ApplyPageScene.ApplyPageSceneBuilder applyBuild = ApplyPageScene.builder();
            ...
            Preconditions.checkArgument(newApplyTotal == applyTotal + 1,
                    "【卡券审核】列表数：" + CommonUtil.result(applyTotal + 1, newApplyTotal));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void voucherManage_data_20() {
        ...
        try {
            ...
            String voucherName = util.getVoucherName(voucherId);
            //发卡记录列表数
            IScene scene = SendRecordScene.builder().build();
    ...
    public void voucherManage_data_21() {
        ...
        try {
            ...
            IScene scene1 = CreateVerificationPeopleScene.builder().verificationPersonName("walawala")
                    .verificationPersonPhone(phone).type(1).status(1).build();
            jc.invokeApi(scene1);
            int newTotal = jc.invokeApi(scene).getInteger("total");
            CommonUtil.valueView(total, newTotal);
            Preconditions.checkArgument(newTotal == total + 1,
                    "列表数：" + CommonUtil.result(total + 1, newTotal));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void voucherManage_data_22() {
        ...
        try {
            ...
            //核销列表
            VerificationRecordScene.VerificationRecordSceneBuilder builder = VerificationRecordScene.builder();
            ...
            Preconditions.checkArgument(verificationChannelName.equals("主动核销"),
                    "核销渠道：" + CommonUtil.result("主动核销", verificationChannelName));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void packageManager_data_1() {
        ...
        try {
            Arrays.stream(EnumSubject.values()).forEach(subject -> {
                ...
                IScene scene1 = CreatePackageScene.builder().packageName(packageName).validity("30")
                ...
                //列表内容校验
                IScene scene2 = PackageFormPageScene.builder().packageName(packageName).build();
    ...
    public void packageManager_data_2() {
        ...
        try {
            //获取售出套数
            IScene scene = PackageFormPageScene.builder().packageName("凯迪拉克无限套餐").build();
            ...
            Preconditions.checkArgument(newSoldNumber == soldNumber + 1,
                    "凯迪拉克无限套餐" + "售出：" + CommonUtil.result(soldNumber + 1, newSoldNumber));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void packageManager_data_3() {
        ...
        try {
            ...
            Preconditions.checkArgument(newGiveNumber == giveNumber + 1,
                    "凯迪拉克无限套餐赠送：" + CommonUtil.result(giveNumber + 1, newGiveNumber));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void packageManager_data_4() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            WechatCustomerPageScene.WechatCustomerPageSceneBuilder builder = WechatCustomerPageScene.builder();
            int total = jc.invokeApi(builder.build()).getInteger("total");
            int s = CommonUtil.getTurningPage(total, size);
            for (int i = 1; i < s; i++) {
                JSONArray array = jc.invokeApi(builder.page(i).size(size).build()).getJSONArray("list");
    ...
    public void packageManager_data_6() {
        ...
        try {
            ...
            PackageFormPageScene.PackageFormPageSceneBuilder builder = PackageFormPageScene.builder().packageStatus(true);
            int total = jc.invokeApi(builder.build()).getInteger("total");
            int s = CommonUtil.getTurningPage(total, size);
    ...
    public void packageManager_data_9() {
        ...
        try {
            //套餐购买记录列表数
            int buyPackageRecordTotal = jc.invokeApi(BuyPackageRecordScene.builder().build()).getInteger("total");
            // 购买固定套餐
            util.buyFixedPackage(1);
            // 确认支付
            util.makeSureBuyPackage("凯迪拉克无限套餐");
            JSONObject response = jc.invokeApi(BuyPackageRecordScene.builder().build());
            int newBuyPackageRecordTotal = response.getInteger("total");
            String sendChannelName = CommonUtil.getStrField(jc.invokeApi(BuyPackageRecordScene.builder().packageName("凯迪拉克无限套餐").build()), 0, "send_channel_name");
    ...
    public void packageManager_data_10() {
        ...
        try {
            ...
            PackageFormPageScene.PackageFormPageSceneBuilder builder = PackageFormPageScene.builder().startTime(startTime).endTime(endTime);
            int total = jc.invokeApi(builder.build()).getInteger("total");
            int s = CommonUtil.getTurningPage(total, size);
            for (int i = 1; i < s; i++) {
                builder.page(i).size(size);
                ...
                array.forEach(e -> {
                    ...
                    BuyPackageRecordScene.BuyPackageRecordSceneBuilder builder1 = BuyPackageRecordScene.builder().packageName(packageName)
    ...
    public void messageManager_data_1() {
        ...
        try {
            ...
            //获取卡券累计发出
            IScene scene = VoucherFormPageScene.builder().voucherName(name).build();
            long cumulativeDelivery = CommonUtil.getIntField(jc.invokeApi(scene), 0, "cumulative_delivery");
            //发卡记录页列表数
            int sendRecordTotal = jc.invokeApi(SendRecordScene.builder().build()).getInteger("total");
            //消息发送一张卡券
            ...
            CommonUtil.valueView(cumulativeDelivery, newCumulativeDelivery);
            Preconditions.checkArgument(newCumulativeDelivery == cumulativeDelivery + 1,
                    "消息发出一张卡券后，" + name + "累计发出数量：" + CommonUtil.result(cumulativeDelivery + 1, newCumulativeDelivery));
            int newSendRecordTotal = jc.invokeApi(SendRecordScene.builder().build()).getInteger("total");
            CommonUtil.valueView(sendRecordTotal, newSendRecordTotal);
            Preconditions.checkArgument(newSendRecordTotal == sendRecordTotal + 1
                    , "发卡记录列表数：" + CommonUtil.result(sendRecordTotal + 1, newSendRecordTotal));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void messageManager_data_2() {
        ...
        try {
            ...
            Preconditions.checkArgument(newMessageTotal == messageTotal + 1
                    , "消息推送后，消息列表数：" + CommonUtil.result(messageTotal + 1, newMessageTotal));
            //消息的状态为发送成功
                ...
                array.forEach(e -> {
                    ...
                    if (pushTime.equals(sendTime)) {
                        ...
                        Preconditions.checkArgument(sendStatusName.equals(EnumSendStatusName.SENT.getName())
                                , "消息状态为：" + CommonUtil.result(EnumSendStatusName.SENT.getName(), sendStatusName));
                        //消息内容校验
                        String content = jsonObject.getString("content");
                        CommonUtil.valueView(content, EnumDesc.ARTICLE_DESC.getDesc());
                        Preconditions.checkArgument(content.equals(EnumDesc.ARTICLE_DESC.getDesc()),
                                "消息内容：" + CommonUtil.result(EnumDesc.ARTICLE_DESC.getDesc(), content));
                    }
                });
            }
        } catch (Exception | AssertionError e) {
    ...
    public void messageManager_data_3() {
        ...
        try {
            ...
            CommonUtil.valueView(messageTotal, newMessageTotal);
            Preconditions.checkArgument(newMessageTotal == messageTotal + 1,
                    "消息定时后，消息列表数：" + CommonUtil.result(messageTotal + 1, newMessageTotal));
                ...
                array.forEach(e -> {
                    ...
                    if (pushTime.equals(sendTime)) {
                        ...
                        Preconditions.checkArgument(sendStatusName.equals(EnumSendStatusName.SCHEDULE.getName())
                                , "消息状态为：" + CommonUtil.result(EnumSendStatusName.SCHEDULE.getName(), sendStatusName));
                    }
                });
            }
        } catch (Exception | AssertionError e) {
            ...
            for (int i = 1; i < s; i++) {
                ...
                for (int j = 0; j < list.size(); j++) {
                    ...
                    Preconditions.checkArgument(statusName.equals(result),
                            i + "页" + (j + 1) + "条" + "发出条数/收到条数=" + percent + CommonUtil.result(result, statusName));
                ...
                list.forEach(e -> {
                    ...
                    if (jsonObject.getString("push_time").equals(date)) {
                        ...
                        Preconditions.checkArgument(platNumber.equals(number),
                                "车牌号" + CommonUtil.result(number, platNumber));
                        Preconditions.checkArgument(customerPhone.equals(marketing.getPhone()),
                                "联系方式" + CommonUtil.result(marketing.getPhone(), customerPhone));
                    }
                });
            }
        } catch (Exception | AssertionError e) {
    ...
    public void voucherManage_system_1() {
        ...
        try {
            ...
            Arrays.stream(strings).forEach(name -> {
                IScene scene = CreateVoucherScene.builder().voucherPic(util.getPicPath()).voucherName(name).subjectType(util.getSubjectType())
    ...
    public void voucherManage_system_2() {
        ...
        try {
            ...
            Arrays.stream(strings).forEach(desc -> {
                IScene scene = CreateVoucherScene.builder().voucherPic(util.getPicPath()).voucherName(util.createVoucherName()).subjectType(util.getSubjectType())
    ...
    public void voucherManage_system_3() {
        ...
        try {
            ...
            Arrays.stream(strings).forEach(subjectType -> {
                IScene scene = CreateVoucherScene.builder().voucherPic(util.getPicPath()).voucherName(util.createVoucherName()).subjectType(subjectType)
    ...
    public void voucherManage_system_5() {
        ...
        logger.logCaseStart(caseResult.getCaseName());
        try {
            Integer[] longs = {1000000000, null, -100};
    ...
    public void voucherManage_system_30() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            String[] strings = {null, EnumDesc.VOUCHER_DESC.getDesc()};
            Arrays.stream(strings).forEach(name -> {
                ...
                Preconditions.checkArgument(message.equals(err),
                        "核销人员名字为：" + name + CommonUtil.result(err, message));
                CommonUtil.logger(name);
            });
        } catch (Exception | AssertionError e) {
            collectMessage(e);
    ...
    public void voucherManage_system_38() {
        ...
        try {
            ...
            Arrays.stream(strings).forEach(phone -> {
                IScene scene = CreateVerificationPeopleScene.builder().verificationPersonName("郭丽雅")
                        .verificationPersonPhone(phone).status(1).type(0).build();
                String message = jc.invokeApi(scene, false).getString("message");
                CommonUtil.valueView(message);
                String err = "手机号格式不正确";
                Preconditions.checkArgument(message.equals(err),
                        "手机号格式为：" + phone + CommonUtil.result(err, message));
    ...
    public void packageManager_system_1() {
        ...
        try {
            ...
            Arrays.stream(strings).forEach(name -> {
                ...
                Preconditions.checkArgument(message.equals(err),
                        "套餐名称为：" + name + CommonUtil.result(err, message));
                CommonUtil.logger(name);
            });
        } catch (Exception | AssertionError e) {
            collectMessage(e);
    ...
    public void packageManager_system_2() {
        ...
        try {
            String[] strings = {null, "", EnumDesc.ARTICLE_DESC.getDesc()};
            Arrays.stream(strings).forEach(desc -> {
                IScene scene = CreatePackageScene.builder().packageName(util.createPackageName()).validity("30").packageDescription(desc)
                        .subjectType(util.getSubjectType()).subjectId(util.getSubjectId(util.getSubjectType()))
                        .voucherList(util.getVoucherInfo()).packagePrice(5000.00).status(true).shopIds(util.getShopIdList()).build();
                String message = jc.invokeApi(scene, false).getString("message");
                CommonUtil.valueView(message);
                ...
                Preconditions.checkArgument(message.equals(err),
                        "套餐说明为：" + desc + CommonUtil.result(err, message));
                CommonUtil.logger(desc);
            });
        } catch (Exception | AssertionError e) {
            collectMessage(e);
    ...
    public void packageManager_system_3() {
        ...
        try {
            ...
            Arrays.stream(strings).forEach(validity -> {
                IScene scene = CreatePackageScene.builder().packageName(util.createPackageName()).validity(validity)
                ...
                Preconditions.checkArgument(message.equals(err),
                        "有效期为：" + validity + CommonUtil.result(err, message));
    ...
    public void packageManager_system_4() {
        ...
        try {
            String[] strings = {"全部权限", null, ""};
            Arrays.stream(strings).forEach(subjectType -> {
                IScene scene = CreatePackageScene.builder().packageName(util.createPackageName()).validity("30")
                        .packageDescription(util.getDesc()).subjectType(subjectType)
    ...
    public void packageManager_system_5() {
        ...
        try {
            ...
            String err = "主体详情不能为空";
            Preconditions.checkArgument(message.equals(err),
                    "主体详情为：" + null + CommonUtil.result(err, message));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void packageManager_system_6() {
        ...
        try {
            ...
            String err = "所选卡券不能为空";
            Preconditions.checkArgument(message.equals(err),
                    "包含卡券为：" + null + CommonUtil.result(err, message));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void packageManager_system_7() {
        ...
        try {
            ...
            Arrays.stream(doubles).forEach(packagePrice -> {
                ...
                Preconditions.checkArgument(message.equals(err),
                        "有效期为：" + packagePrice + CommonUtil.result(err, message));
    ...
    public void packageManager_system_8() {
        ...
        try {
            ...
            Preconditions.checkArgument(message.equals(err),
                    "选择门店为：" + null + CommonUtil.result(err, message));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    @Test(description = "套餐表单--【创建套餐】选择卡券后将卡券作废，点击确定，提示：卡券【xxxx】已被作废，请重新选择！")
    public void packageManager_system_29() {
        ...
        try {
            ...
            saveData("套餐表单--【创建套餐】选择卡券后将卡券作废，点击确定，提示：卡券【xxxx】已被作废，请重新选择！");
        }
    }
    ...
    public void packageManager_system_18() {
        try {
            ...
            IScene purchaseTemporaryPackageScene = PurchaseTemporaryPackageScene.builder().customerPhone(marketing.getPhone())
                    .carType(EnumCarType.RECEPTION_CAR.name()).plateNumber(util.getPlatNumber(marketing.getPhone()))
                    .voucherList(voucherList).expiryDate("1").remark(EnumDesc.VOUCHER_DESC.getDesc()).subjectType(subjectType)
    ...
    public void packageManager_system_21() {
        ...
        try {
            ...
            String message = jc.invokeApi(purchaseFixedPackageScene, false).getString("message");
            String err = "此套餐已关闭";
            Preconditions.checkArgument(message.contains(err), "购买的套餐内包含无库存卡券" + CommonUtil.result(err, message));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
    ...
    public void packageManager_system_22() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            String[] packageNames = {EnumVP.ONE.getPackageName(), EnumDesc.PACKAGE_DESC.getDesc(), null, ""};
    ...
    public void packageManager_system_23() {
        ...
        try {
            ...
            Arrays.stream(contents).forEach(content -> {
                IScene scene = EditPackageScene.builder().packageName(packageName).packageDescription(content).validity(2000)
                        .subjectType(util.getSubjectType()).subjectId(util.getSubjectId(util.getSubjectType()))
                        .voucherList(util.getVoucherInfo()).packagePrice("1.11").status(true).shopIds(util.getShopIdList())
                        .id(String.valueOf(packageId)).build();
                String message = jc.invokeApi(scene, false).getString("message");
    ...
    public void messageManager_system_12() {
        ...
        try {
            ...
            Preconditions.checkArgument(message.equals(err), "选择套餐时，套餐被关闭" + CommonUtil.result(err, message));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {
            jc.pcSwitchPackageStatus(true, packageId);
    ...
    public void messageManager_system_13() {
        ...
        try {
            ...
            Preconditions.checkArgument(message.equals(err), "选择套餐时，套餐内包含无库存卡券" + CommonUtil.result(err, message));
        } catch (Exception | AssertionError e) {
            collectMessage(e);
        } finally {

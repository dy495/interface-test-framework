...
//                sr.finish_end =data.getString(ss[7][1].toString());
//                sr.finish_start=data.getString(ss[8][1].toString());
//                sr.reception_end =data.getString(ss[9][1].toString());
//                sr.reception_start=data.getString(ss[10][1].toString());
                sr.page="1";
                sr.size="10";
...
//                Preconditions.checkArgument(result.getString(ss[7][1].toString()).contains(sr.finish_end),"参数全部输入的查询的"+sr.finish_end+"与列表信息的第一行"+result.getString(ss[7][1].toString()+"不一致"));
//                Preconditions.checkArgument(result.getString(ss[8][1].toString()).contains(sr.finish_start),"参数全部输入的查询的"+sr.finish_start+"与列表信息的第一行的"+result.getString(ss[8][1].toString()+"不一致"));
//                Preconditions.checkArgument(result.getString(ss[9][1].toString()).contains(sr.reception_end),"参数全部输入的查询的"+sr.reception_end+"与列表信息的第一行"+result.getString(ss[9][1].toString()+"不一致"));
//                Preconditions.checkArgument(result.getString(ss[10][1].toString()).contains(sr.reception_start),"参数全部输入的查询的"+sr.reception_start+"与列表信息的第一行的"+result.getString(ss[10][1].toString()+"不一致"));
...
//        Preconditions.checkArgument(createDate.compareTo(startDate)>=0&&createDate.compareTo(endDate)<=0, "列表中创建时间："+createDate+"筛选栏开始时间："+startTime+"筛选栏开始时间："+endTime+"创建时间不在开始系欸结束之间之间");
    ...
    /**
     * @description :销售客户查询-筛选栏单项查询--没有加时间筛选
     * @date :2020/11/24
     **/
    ...
    public void verificationRecordOneFilter(String pram,String output){
        ...
        try{
            ...
            if(respon.getJSONArray("list").size()>0){
                ...
                for(int page=1;page<=pages;page++){
                    for(int i=0;i<list.size();i++){
                        System.out.println(pram+"++++++++"+respon.getJSONArray("list").getJSONObject(i).getString("sender"));
                        String Flag=list.getJSONObject(i).getString(output);
...
//                Preconditions.checkArgument(result.getJSONObject(i).getString(String.valueOf(flag[2][1])).contains(variable.verification_code),"参数全部输入的查询的"+variable.verification_code+"与列表信息的第一行的"+result.getJSONObject(i).getString(flag[2][1].toString()+"不一致"));
    ...
    /**
     * @description :套餐购买记录-筛选栏单项查询---发套餐者搜索，服务端筛选返回有问题
     * @date :2020/11/24
     **/
...
//                Preconditions.checkArgument(result.getJSONObject(i).getString(String.valueOf(flag[2][1])).contains(variable.send_type),"参数全部输入的查询的"+variable.send_type+"与列表信息的第一行的"+result.getJSONObject(i).getString(flag[0][1].toString()+"不一致"));
    ...
    public void messageFormOneFilter(String pram,String output){
        ...
        try{
            ...
            if(respon.getJSONArray("list").size()>0&&result!=null){
                JSONArray list=respon1.getJSONArray("list");
                int pages=respon1.getInteger("pages");
                if(pram.equals("message_type")){
                    for(int page=1;page<=pages;page++){
                        for(int i=0;i<list.size();i++){
                            ...
                            Preconditions.checkArgument(Flag.contains(sendAccount), "消息表单管理按"+sendAccount+"查询结果"+Flag);
                        }
                    }
                }else{
                    for(int page=1;page<=pages;page++){
        ...
        try{
            ...
            if(respon.getJSONArray("list").size()>0){
                ...
                if(pram.equals("status")){
                    ...
                    JSONObject respon1=jc.applyListFilterManage(shopId,"1","10",pram,statusName);
                    JSONArray list=respon1.getJSONArray("list");
                    for(int page=1;page<=pages;page++){
                        for(int i=0;i<list.size();i++){
                            String Flag=list.getJSONObject(i).getString(output);

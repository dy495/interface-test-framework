package com.haisheng.framework.testng.bigScreen.itemMall.casedaily;
...
public class MallDataCenter extends TestCaseCommon implements TestCaseStd {
    ...
    public void initial() {
        ...
        commonConfig.checklistCiCmd = commonConfig.checklistCiCmd.replace(commonConfig.JOB_NAME, "shopMall-daily-test");

        //replace product name for ding push
        commonConfig.message = commonConfig.message.replace(commonConfig.TEST_PRODUCT, "购物中心 日常");
...
//    }
     ...
     */
    @Test
    public void mall_accountAdd_Phone() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
...
//            //新增账号时，添加员工姓名长度的大于20
            ...
            JSONObject res3 = mall.organizationAccountAddTwo("", EnumDesc.DESC_BETWEEN_5_10.getDesc(), "123456", "uid_ef6d2de5", type0, null, null, status, roleIdList, shopIdList);
            String message3 = res3.getString("message");
            Preconditions.checkArgument(message3.equals("登陆邮箱不可为空"), "邮箱为空也成功了" + res3.getString("message"));
...
//    @Test
    public void mall_role_add_error() {
        ...
        try {
            ...
            int superior_role_id = 6339;
            //角色名称大于20
            JSONObject res = mall.organizationRoleAddTwo(EnumDesc.DESC_BETWEEN_20_30.getDesc(), superior_role_id, description, moduleId);
            String message = res.getString("message");
            checkArgument(message.equals("角色名称需要在1-20个字内"), "角色名称大于20也成功了" + res.getString("message"));
...
//            saveData("新增角色异常情况");
    ...
    public void mall_seach(String name, String email, String phone, String role_name, String shopname) {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            JSONArray list = mall.organizationAccountPage(name, "", "", "", "", "", page, size).getJSONArray("list");
            for (int i = 0; i < list.size(); i++) {
                String name1 = list.getJSONObject(i).getString("name");
                Preconditions.checkArgument(name1.contains(name), "通过" + name + "搜索展示账号为" + name1);
            }
            JSONArray list0 = mall.organizationAccountPage("", "", email, "", "", "", page, size).getJSONArray("list");
            for (int i = 0; i < list0.size(); i++) {
                String email1 = list0.getJSONObject(i).getString("email");
                Preconditions.checkArgument(email1.contains(email), "通过" + email + "搜索展示账号为" + email1);
            }
            JSONArray list1 = mall.organizationAccountPage("", "", "", phone, "", "", page, size).getJSONArray("list");
            for (int i = 0; i < list1.size(); i++) {
                String phone1 = list1.getJSONObject(i).getString("phone");
                Preconditions.checkArgument(phone1.contains(phone), "通过" + phone + "搜索展示账号为" + phone1);
            }
            JSONArray list2 = mall.organizationAccountPage("", "", "", "", role_name, "", page, size).getJSONArray("list");
            for (int i = 0; i < list2.size(); i++) {
                String role_name1 = list2.getJSONObject(i).getJSONArray("role_list").getJSONObject(0).getString("role_name");
                Preconditions.checkArgument(role_name1.contains(role_name), "通过" + role_name + "搜索展示账号为" + role_name1);
            }
            JSONArray list3 = mall.organizationAccountPage("", "", "", "", "", shopname, page, size).getJSONArray("list");
            for (int i = 0; i < list3.size(); i++) {
                String shop_name = list3.getJSONObject(i).getJSONArray("shop_list").getJSONObject(0).getString("name");
                Preconditions.checkArgument(shop_name.contains(shopname), "通过" + shopname + "搜索展示账号为" + shop_name);
            }
        } catch (AssertionError | Exception e) {
            appendFailReason(e.toString());
        } finally {
//            saveData("账号管理搜索框");
    ...
    public Object[] name() {
        return new String[][]{
                {"越", "yuexiu@test.com", "18513118484", "总管理员", "购物中心"},
                {"越秀测试账号", "yuexiu", "185", "总管理员", "购物中心日常测试"},
                {"客流员工124", "yuexiu@test.com111", "123123213", "123123", ""},
        };
    }

    @Test
    public void mall_role_seach() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            JSONArray role_list = mall.organizationRoleList().getJSONArray("list");
            for (int i = 0; i < role_list.size(); i++) {
                String rolename = role_list.getJSONObject(i).getString("role_name");
                JSONArray list = mall.organizationRolePage(rolename, page, size).getJSONArray("list");
                for (int j = 0; j < list.size(); j++) {
                    String role_name = list.getJSONObject(j).getString("role_name");
                    Preconditions.checkArgument(rolename.equals(role_name), "通过" + rolename + "搜索展示角色为" + role_name);
                }
            }
        } catch (AssertionError | Exception e) {
            appendFailReason(e.toString());
    ...
    //按照楼层搜索门店
    @Test
    public void mall_shop_seach() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            JSONArray floor_list = mall.floor_list().getJSONArray("list");
            for (int i = 0; i < floor_list.size(); i++) {
                int floolid = floor_list.getJSONObject(i).getInteger("id");
                JSONArray list = mall.shop_list(null, floolid, page, size).getJSONArray("list");
                for (int j = 0; j < list.size(); j++) {
                    int id = list.getJSONObject(j).getInteger("floor_id");
                    Preconditions.checkArgument(floolid == id, "通过楼层id为" + floolid + "搜索展示楼层id为" + id);
                }
            }
        } catch (AssertionError | Exception e) {
            appendFailReason(e.toString());
    ...
    //按照名称模糊搜索门店
    @Test
    public void mall_shop_seach0() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            JSONArray list = mall.shop_list("楼层区域", null, page, size).getJSONArray("list");
            for (int j = 0; j < list.size(); j++) {
                String name = list.getJSONObject(j).getString("name");
                Preconditions.checkArgument(name.contains("楼层区域"), "通过楼层名称为楼层区域搜索展示楼层名称为" + name);
            }
        } catch (AssertionError | Exception e) {
            appendFailReason(e.toString());
        } finally {
    ...
    //按照名称精确搜索门店
    @Test
    public void mall_shop_seach1() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            JSONArray list = mall.shop_list("楼层区域:B1", null, page, size).getJSONArray("list");
            for (int j = 0; j < list.size(); j++) {
                String name = list.getJSONObject(j).getString("name");
                Preconditions.checkArgument(name.contains("楼层区域:B1"), "通过楼层名称为楼层区域搜索展示楼层名称为" + name);
            }
        } catch (AssertionError | Exception e) {
            appendFailReason(e.toString());
        } finally {
    ...
    //按照名称和楼层搜索门店
    @Test
    public void mall_shop_seach2() {
        logger.logCaseStart(caseResult.getCaseName());
        try {
            JSONArray list = mall.shop_list("楼层区域:B1", 7343, page, size).getJSONArray("list");
            for (int j = 0; j < list.size(); j++) {
                String name = list.getJSONObject(j).getString("name");
                int id = list.getJSONObject(j).getInteger("floor_id");
                Preconditions.checkArgument(name.contains("楼层区域:B1") && id == 7343, "通过楼层名称为楼层区域搜索展示楼层名称为" + name + "输入楼层id为7343" + "展示id为" + id);
            }
        } catch (AssertionError | Exception e) {
            appendFailReason(e.toString());
        } finally {
            saveData("通过门店名称+楼层搜索门店");
        }
    }
}

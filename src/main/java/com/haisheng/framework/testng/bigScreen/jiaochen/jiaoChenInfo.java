...
public class jiaoChenInfo {
    ...
    public final String sitphone2 = "0433-381"+Integer.toString((int)((Math.random()*9+1)*1000)); ;//座机号
...
//        Long shopId  = AppletCommonShopListScene.builder().carModelId(modelId).coordinate(JSONArray.parseArray(str)).build().invoke(visitor).getJSONArray("list").getJSONObject(0).getLong("shop_id");
    ...
    public void newBuyCarRec(){
        ...
        jc.createCstm(name,phone,type,sex,car_style_id,car_model_id,shop_id,salesId,dt.getHistoryDate(0),"ASDFUGGDSF99"+Integer.toString((int)((Math.random()*9+1)*10000)),false).getInteger("code");

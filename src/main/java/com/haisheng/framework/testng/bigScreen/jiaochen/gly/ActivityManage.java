...
import com.haisheng.framework.testng.bigScreen.itemPorsche.casedaily.wm.base.mapper.CustomerPageEnum;
...
public class ActivityManage extends TestCaseCommon implements TestCaseStd {
    ...
    public void fissionVoucherMainProcess() {
        ...
        try {
            ...
            //再次创建裂变活动
            Long voucherId2 = businessUtil.getVoucherId();
    ...
    public void RecruitActivityTitleException13() {
        ...
        try {
            ...
            for (int i = 0; i < num.length; i++) {
                ...
                IScene scene = builder.build();
                String message = visitor.invokeApi(scene, false).getString("message");
                Preconditions.checkArgument(message.equals("奖励数量不能为空") || message.equals("奖励数量不能为空") || message.contains("可用库存不足") || message.equals("奖励数量至少一张"), "被邀请者优惠券配置异常情况为：" + num[i]);
            }

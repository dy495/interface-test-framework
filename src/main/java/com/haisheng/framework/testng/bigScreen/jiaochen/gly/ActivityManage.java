...
public class ActivityManage extends TestCaseCommon implements TestCaseStd {
    ...
    private static final EnumAccount account = EnumAccount.JC_DAILY_YS;
    private static final EnumAppletToken APPLET_USER = EnumAppletToken.JC_GLY_DAILY;
    public VisitorProxy visitor = new VisitorProxy(product);
    BusinessUtil util = new BusinessUtil(visitor);
    ...
    public void fissionVoucherMainProcess() {
        ...
        try {
            ...
            //再次创建裂变活动
            Long voucherId2 = util.getVoucherId();
            ...
            System.err.println(activityId2 + "---------" + statusPassed + "-------" + message);
            //置顶
            ActivityManageTopScene.builder().id(activityId2).build().visitor(visitor).execute();
            //获取此活动的名称
            String title = util.getRecruitActivityDetailDate1(activityId2).getString("title");
            System.err.println("----------title:" + title);
            //登录小程序
            util.loginApplet(EnumAppletToken.JC_GLY_DAILY);
    ...
    public void recruitMainProcess() {
        ...
        try {
            ...
            //置顶
            ActivityManageTopScene.builder().id(activityId).build().visitor(visitor).execute();
            //获取活动的状态
            ...
            util.loginApplet(EnumAppletToken.JC_GLY_DAILY);
            //小程序报名此活动
            util.activityRegisterApplet(activityId, "13436941018", "雪晴", 2, "1513814362@qq.com", "22", "女", "其他", title);
            //登录PC
            jc.pcLogin(pp.phone1, pp.password);
            //审批通过小程序活动报名
    ...
    public void preCreateSomeActivity() {
        try {
            ...
            for (int i = 0; i < 1; i++) {
                jc.pcLogin(pp.phone1, pp.password);
                //创建营销活动
                Long id1 = util.getContentMarketingAdd();
                //创建裂变活动
                Long id2 = util.createFissionActivity(voucherId);
                util.getApprovalPassed(id1, id2);
                //创建招募活动
                Long activityId = util.createRecruitActivity(voucherId, true, 0, true);
                //审批通过
                util.getApprovalPassed(activityId);
                //置顶
                Long[] ids = {id1, id2, activityId};
                Arrays.stream(ids).forEach(id -> ActivityManageTopScene.builder().id(id).build().visitor(visitor).execute());
                //登陆小程序
                util.loginApplet(EnumAppletToken.JC_GLY_DAILY);
...
//            String parValue = businessUtil.getPrice(voucherId);
            ...
            //填写库存数量
            String number = String.valueOf(util.getVoucherAllowUseInventoryNum(voucherId));
...
//            Preconditions.checkArgument(price.equals(parValue),"优惠券的面值和活动中优惠券的面值不一致");
    ...
    public void activityRegisterDate9() {
        ...
        try {
            ...
            Long activityId;
            //获取进行中的活动存在待审批数量的ID
            ManagePageBean managePageBean = util.getRecruitActivityWorkingApproval();
            if (managePageBean == null) {
                activityId = util.getRecruitActivityWorkingApprovalList().get(0).getId();
                util.loginApplet(APPLET_USER);
                util.appletSignUpActivity(activityId);
            } else {
                activityId = managePageBean.getId();
            }
            //审批通过之前报名成功的数量
            int passedBefore = util.getRegisterData(activityId).getInteger("passed");
            //报名待审批的ID合集
            List<Long> idArray = util.registerApproval(activityId);
            //审批通过其中一条报名
            util.getRegisterApprovalPassed(activityId, idArray.get(0));
            //审批通过的活动人数
    ...
    /**
     * 报名列表-审批通过1条，报名成功&报名成功列表--报名人数不填写     ok
     */
    @Test()
    public void activityRegisterDate91() {
        ...
        try {
            ...
            //审批通过之后报名成功的数量
            int passedAfter = util.getRegisterData(activityId).getInteger("passed");
            System.out.println(passedAfter + "--------" + passedBefore);
    ...
    public void activityRegisterDate10() {
        ...
        try {
            ...
            //审批通过之前报名成功的数量
            int failedBefore = util.getRegisterData(activityId).getInteger("failed");
            //报名待审批的ID合集
            List<Long> idArray = util.registerApproval(activityId);
            //审批不通过其中一条报名
            util.getRegisterApprovalReject(activityId, idArray.get(0));
            //审批通过的活动人数
            int pages = util.getRegisterPage(activityId).getInteger("pages");
            for (int page = 1; page <= pages; page++) {
                IScene scene = ManageRegisterPageScene.builder().page(page).size(10).activityId(activityId).build();
            ...
            //审批通过之后报名成功的数量
            int failedAfter = util.getRegisterData(activityId).getInteger("failed");
    ...
    public void activityApproval15() {
        ...
        try {
            ...
            //获取进行中的活动存在待审批ID合集
            Long id = util.getActivityWait().getId();
            //审批通过其中一条
    ...
    public void activityApproval() {
        ...
        try {
            ...
            //获取刚才通过的活动的状态
            int status = util.getActivityApprovalStatus(id);
    ...
    public void activityNotApproval() {
        ...
        try {
            //获取列表中状态为【待审批】的活动ID
            Long id = util.getActivityWaitingApproval().getId();
            //审批不通过待审批的活动
            util.getApprovalReject(id);
            //获取刚才不通过的活动的状态
            int status = util.getActivityStatus(id);
    ...
    public void revokeApprovalsActivity() {
        ...
        try {
            //获取待审批活动的ID
            Long id = util.getRecruitActivityWaitingApproval().getId();
            //撤回待审批的活动
            String message = util.getRevokeActivity(id);
            //获取刚才通过的活动的状态
            int status = util.getActivityStatus(id);
            Preconditions.checkArgument(status == ActivityStatusEnum.REVOKE.getId(), "撤回的待审批的活动,现活动的状态为：" + status);
    ...
    public void rejectActivityEdit() {
        ...
        try {
            ...
            System.out.println(title + "--------" + rule + "--------" + participationLimitType + "--------" + startDate + "--------" + endDate + "--------" + applyStart + "--------" + applyEnd + "--------" + quota + "--------" + subjectType + "--------" + label + "--------" + address + "--------" + approval + "--------" + aLong + "--------" + voucherId);
...
//            Preconditions.checkArgument(picName.contains("活动.jpeg"), "审核未通过的活动编辑失败");
    ...
    public void promotionWaitingStarActivity() {
        ...
        try {
            ...
            //推广未开始的活动
            String appletCodeUrl = util.getPromotionActivity(id);
            System.err.println(appletCodeUrl + "--------" + id);
    ...
    public void revokeApprovalsFissionActivity() {
        ...
        try {
            //获取待审批活动的ID
            Long id = util.getFissionActivityWaitingApproval().getId();
            //撤回待审批的活动
            util.getRevokeActivity(id);
            //获取刚才通过的活动的状态
            int status = util.getActivityStatus(id);
    ...
    public void revokeFissionActivityDelete() {
        ...
        try {
            //获取已撤销活动的ID
            Long id = util.getFissionActivityRevoke().getId();
            //删除已撤销的活动
            String message = util.getDelActivity(id);
            Preconditions.checkArgument(message.equals("success"), "已撤销的活动删除失败");
        } catch (AssertionError | Exception e) {
...
//            Preconditions.checkArgument(picName.contains("活动.jpeg"), "已撤销的活动编辑失败");
    ...
    public void rejectFissionActivityEdit() {
        ...
        try {
            ...
            picList.add(util.getPicPath());
            // 创建被邀请者和分享者的信息字段
            JSONObject invitedVoucher = util.getInvitedVoucher(voucherId, 1, String.valueOf(Math.min(util.getVoucherAllowUseInventory(voucherId), 2)), 2, "", "", 1);
            JSONObject shareVoucher = util.getShareVoucher(voucherId, 1, String.valueOf(Math.min(util.getVoucherAllowUseInventory(voucherId), 2)), 2, "", "", 1);
            //所属主体
            String subject = supporterUtil.getSubjectType();
            //开始时间和结束时间
            String startTime = util.getStartDate();
...
//            String content = businessUtil.changeRecordPage(id).getJSONArray("list").getJSONObject(0).getString("content");    &&content.equals("编辑活动")
    ...
    public void RecruitActivityTitleException13() {
        ...
        try {
            ...
            for (String s : num) {
                ...
                IScene scene = builder.build();
                String message = scene.visitor(visitor).getResponse().getMessage();
                Preconditions.checkArgument(message.equals("奖励数量不能为空") || message.contains("可用库存不足") || message.equals("奖励数量至少一张"), "被邀请者优惠券配置异常情况为：" + s);
            }
        } catch (AssertionError | Exception e) {
            collectMessage(e);
        } finally {
    ...
    public void RecruitActivityTitleException14() {
        ...
        try {
            ...
            //创建招募活动-共有的--基础信息
            util.getSurplusInventory(voucherId);
            JSONArray registerInformationList = this.util.getRegisterInformationList(isShow, isRequired);
            //报名成功奖励
            JSONArray registerObject = util.getRewardVouchers(voucherId, 1, util.getVoucherAllowUseInventory(voucherId));
            ManageRecruitAddScene.ManageRecruitAddSceneBuilder builder = ManageRecruitAddScene.builder()
    ...
    public void activityStopVoucher() {
        try {
            ...
            String message = scene.visitor(visitor).getResponse().getMessage();
            System.out.println(message);
            Preconditions.checkArgument(message.contains("可用库存不足") || message.contains("暂停发放"), "暂停发放优惠券的校验，message返回结果为：" + message);
        } catch (AssertionError | Exception e) {
            collectMessage(e);
        } finally {
    ...
    public void activityRegistrationNUll() {
        try {
            ...
            builder.rewardVouchers(registerObject)
                    .voucherValid(voucherValid);
            Long activityId = builder.build().visitor(visitor).execute().getLong("id");
            //审批通过招募活动
            util.getApprovalPassed(activityId);
            ActivityManageTopScene.builder().id(activityId).build().visitor(visitor).execute();
            //小程序报名
    ...
    public void activityVoucherStatus1() {
        try {
            ...
            Preconditions.checkArgument(isReceived.equals("true") && code.equals(voucherCode), "自动发放的卡券在小程序的小喇叭中没有被领取||发放的卡券没有到账");
        } catch (AssertionError | Exception e) {
            collectMessage(e);
        } finally {
    ...
    public void activityVoucherStatus2() {
        try {
            ...
            //小程序报名活动(报名信息不填写)
            util.activityRegisterApplet(activityId, "", "", 1, "", "", "", "", title);
            jc.pcLogin(pp.phone1, pp.password);
            //获取报名管理中的信息
            IScene scene3 = ManageRegisterPageScene.builder().page(1).size(10).activityId(activityId).build();
            Long registerId = scene3.visitor(visitor).execute().getJSONArray("list").getJSONObject(0).getLong("id");
            ...
            Long transId = util.appointmentActivityId(activityId);
            //登录小程序
            util.loginApplet(EnumAppletToken.JC_GLY_DAILY);
            //查看小程序中此活动对应的小喇叭中的卡券的状态
    ...
    public void ContentMarketingAdd() {
        ...
        try {
            ...
            for (String s : flag) {
                if (s.equals("012345678901234567891")) {
...
//            String picPath=object.getJSONArray("pic_list").getJSONObject(0).getString("pic_path");
    ...
    public void cancelFissionActivityRecover() {
        ...
        try {
            ...
            Long id = managePageBean.getId();
            //获取活动的状态
            int statusCancel = managePageBean.getStatus();
            //恢复活动并获取状态
            String message = util.getContentMarketingRecover(id);
            int statusRecover = util.getActivityStatus(id);
            Preconditions.checkArgument(message.equals("success"), "活动恢复失败");

...
public class FinishReceptionDataOnline extends TestCaseCommon implements TestCaseStd {
    ...
    public void pcAppointmentTotal() {
        try {
            ...
            Preconditions.checkArgument(pcReceptionTotal1.equals(pcReceptionTotal), "接待前pc接待列表数据为：" + pcReceptionTotal + " 接待后pc接待列表数据为：" + pcReceptionTotal1);
    ...
    @Test(description = "完成接待后，app今日任务分母不变、分母-1")
    public void appTodayData() {
        try {
            ...
            Preconditions.checkArgument(integers1[1].equals(integers[1]), "接待前今日任务销售接待分母为：" + integers[1] + " 接待后今日任务销售接待分母为：" + integers1[1]);
        } catch (AssertionError | Exception e) {
            collectMessage(e);
        } finally {
            saveData("完成接待后，app今日任务分母不变、分母-1");
        }
    }

    @Test(description = "今日任务和今日数据比较")
    public void checkFinishTodayTaskAndTodayData() {
        try {
            AppTodayTaskDto appTodayTask = util.getAppTodayTask();
            AppTodayDataDto appTodayData = util.getAppTodayData();

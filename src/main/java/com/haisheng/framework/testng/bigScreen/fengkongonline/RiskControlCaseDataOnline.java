...
public class RiskControlCaseDataOnline extends TestCaseCommon implements TestCaseStd {
    ...
    public void authCashierPageData2() {
        try {
            //获取固定的账号，是以前的创建的数据  (15037286013)
            String id = "uid_edfe23f0";
            //获取账号管理页面的账号数量
            JSONObject response = com.haisheng.framework.testng.bigScreen.fengkongdaily.scene.auth.staff.PageScene.builder().page(1).size(10).build().visitor(visitor).execute();
        ...
        try {
            ...
            for (int page = 1; page <= pages; page++) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    //待处理
                    int pendingTotal = RiskEventPageScene.builder().shopId(id).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getInteger("total");
                    //已处理
                    int processedTotal = RiskEventPageScene.builder().shopId(id).page(1).size(10).currentState("PROCESSED").build().visitor(visitor).execute().getInteger("total");
                    //已过期
                    int ExpiredTotal = RiskEventPageScene.builder().shopId(id).page(1).size(10).currentState("EXPIRED").build().visitor(visitor).execute().getInteger("total");
    ...
    public void riskControlData8() {
        try {
            ...
            int pages = scene.visitor(visitor).execute().getInteger("pages") > 3 ? 3 : scene.visitor(visitor).execute().getInteger("pages");
            for (int page = 1; page <= pages; page++) {
                IScene scene1 = com.haisheng.framework.testng.bigScreen.fengkongdaily.scene.auth.cashier.PageScene.builder().page(page).size(10).build();
                JSONArray list = scene1.visitor(visitor).execute().getJSONArray("list");
                for (int i = 0; i < list.size(); i++) {
                    ...
                    //已处理
                    int processed = RiskEventPageScene.builder().shopId(id).page(1).size(10).currentState("PROCESSED").build().visitor(visitor).execute().getInteger("total");
                    //待处理
                    int pendingTotal = RiskEventPageScene.builder().shopId(id).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getInteger("total");
                    //收银风控事件里面正常的事件
                    int normal = RiskEventPageScene.builder().shopId(id).page(1).size(10).handleResult("NORMAL").build().visitor(visitor).execute().getInteger("total");
                    //收银风控事件里面异常的事件
                    int abnormal = RiskEventPageScene.builder().shopId(id).page(1).size(10).handleResult("ABNORMAL").build().visitor(visitor).execute().getInteger("total");
    ...
    public void riskControlData10() {
        try {
            ...
            //待处理
            int pendingTotal = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getInteger("total");
            //收银风控事件里面正常的事件
            int normalTotal = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).handleResult("NORMAL").build().visitor(visitor).execute().getInteger("total");

            //获取待处理风险事件ID
            JSONArray list = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getJSONArray("list");
            long id = list.getJSONObject(0).getInteger("id");
            String order = list.getJSONObject(0).getString("order_id");

            //将待处理的风控事件处理成正常
            String message = cu.getRiskEventHandle(id, 1, "订单正常", null).getMessage();
            ...
            //待处理
            int pendingTotal1 = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getInteger("total");
            //收银风控事件里面正常的事件
            int normalTotal1 = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).handleResult("NORMAL").build().visitor(visitor).execute().getInteger("total");
            //累计风险数量
    ...
    public void riskControlData11() {
        try {
            ...
            int pendingTotal = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getInteger("total");
            //收银风控事件里面异常的事件
            int abnormalTotal = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).handleResult("ABNORMAL").build().visitor(visitor).execute().getInteger("total");
            ...
            //将待处理的风控事件处理成异常
            String message = cu.getRiskEventHandle(id, 0, "订单正常", null).getMessage();
            ...
            int pendingTotal1 = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute().getInteger("total");
            //收银风控事件里面异常的事件
            int abnormalTotal1 = RiskEventPageScene.builder().shopId(shopId).page(1).size(10).handleResult("ABNORMAL").build().visitor(visitor).execute().getInteger("total");
    ...
    public void riskControlData12() {
        ...
        try {
            ...
            for (int page = 1; page <= pages; page++) {
                JSONArray list = RiskEventPageScene.builder().shopId(shopId).page(page).size(10).currentState("PENDING").build().visitor(visitor).execute().getJSONArray("list");
                for (int i = 0; i < list.size(); i++) {
...
//            String updateUser=response.getJSONArray("list").getJSONObject(0).getString("update_user");
    ...
    public void riskControlData19() {
        ...
        try {
            ...
            //待处理事件的ID
            JSONObject response1 = RiskEventPageScene.builder().shopId(id).page(1).size(10).currentState("PENDING").build().visitor(visitor).execute();
            ...
            if (list.size() > 0) {
                ...
                cu.getRiskEventHandle(pendId, 2, "订单异常，请关注", customerIds);
                //获取黑名单列表的条数
    ...
    public void riskControlData22() {
        ...
        try {
            ...
            int total = scene.visitor(visitor).execute().getInteger("tatal");

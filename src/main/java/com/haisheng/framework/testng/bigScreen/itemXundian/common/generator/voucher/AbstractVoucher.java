...
public abstract class AbstractVoucher extends BaseGenerator implements IVoucher {
    ...
    private VoucherFormVoucherPageBean getVoucher() {
        try {
            ...
            VoucherFormVoucherPageBean voucherPage = getPage();
            if (voucherPage != null) {
    ...
    private VoucherFormVoucherPageBean getPage() {
        ...
        JSONObject response = VoucherFormVoucherPageScene.builder().build().execute(visitor);
        int total = response.getInteger("total");
        int s = CommonUtil.getTurningPage(total, SIZE);
        for (int i = 1; i < s; i++) {
            JSONArray array = VoucherFormVoucherPageScene.builder().page(i).size(SIZE).build().execute(visitor).getJSONArray("list");

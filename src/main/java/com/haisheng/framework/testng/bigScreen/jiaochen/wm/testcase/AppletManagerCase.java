...
public class AppletManagerCase extends TestCaseCommon implements TestCaseStd {
    ...
    public void appointmentManager_maintain() {
        ...
        try {
            int i = 0;
            Date date = DateTimeUtil.addDay(new Date(), i);
            Long shopId = util.getShopId();
            IScene appointmentPageScene = AppointmentPageScene.builder().type(AppointmentTypeEnum.MAINTAIN.name()).build();
            int appointmentPageTotal = appointmentPageScene.invoke(visitor).getInteger("total");
            int appointmentNumber = util.appointmentNumber(date, AppointmentTypeEnum.MAINTAIN.name());
            ...
            AppAppointmentPage appAppointmentPage = util.collectBean(AppAppointmentPageScene.builder().build().invoke(visitor).getJSONArray("list").getJSONObject(0), AppAppointmentPage.class);
            CommonUtil.checkResult("app预约类型", AppointmentTypeEnum.MAINTAIN.getValue(), appAppointmentPage.getTypeName());
            CommonUtil.checkResult("app预约类型", AppointmentTypeEnum.MAINTAIN.name(), appAppointmentPage.getType());
            ...
            user.loginPc(ALL_AUTHORITY);
            int newAppointmentNumber = util.appointmentNumber(date, AppointmentTypeEnum.MAINTAIN.name());
            ...
            AppointmentRecordAppointmentPageBean appointmentPage = util.getAppointmentPageById(appointmentId, AppointmentTypeEnum.MAINTAIN.name());
            ...
            user.loginPc(ALL_AUTHORITY);
            AppointmentRecordAppointmentPageBean newAppointmentPage = util.getAppointmentPageById(appointmentId, AppointmentTypeEnum.MAINTAIN.name());
            ...
            EvaluatePageBean evaluatePageBean = util.collectBean(evaluatePageObject.getJSONArray("list").getJSONObject(0), EvaluatePageBean.class);
            ...
            user.loginPc(ALL_AUTHORITY);
            EvaluatePageBean followEvaluatePage = util.collectBean(evaluatePageScene.invoke(visitor).getJSONArray("list").getJSONObject(0), EvaluatePageBean.class);
...
//            int appointmentPageTotal = appointmentPageScene.invoke(visitor).getInteger("total");
//            int appointmentNumber = util.appointmentNumber(date, AppointmentTypeEnum.REPAIR.name());
//            user.loginApp(ALL_AUTHORITY);
//            int appAppointmentNum = util.getAppointmentPageNum();
//            //预约维修
//            user.loginApplet(APPLET_USER_ONE);
...
//            CommonUtil.checkResult("app我的预约列表数", appAppointmentNum + 1, newAppAppointmentNum);
//            user.loginPc(ALL_AUTHORITY);
//            int newAppointmentNumber = util.appointmentNumber(date, AppointmentTypeEnum.REPAIR.name());
//            CommonUtil.checkResult("pc预约看板分子数", appointmentNumber + 1, newAppointmentNumber);
//            int newAppointmentPageTotal = appointmentPageScene.invoke(visitor).getInteger("total");
//            CommonUtil.checkResult("pc预约记录列表数", appointmentPageTotal + 1, newAppointmentPageTotal);
//            AppointmentRecordAppointmentPageBean appointmentPage = util.getAppointmentPageById(appointmentId, AppointmentTypeEnum.REPAIR.name());
...
//            CommonUtil.checkResult("app预约记录数", makeSureAppointmentNum, newMakeSureAppAppointmentNum);
//            user.loginPc(ALL_AUTHORITY);
//            AppointmentRecordAppointmentPageBean newAppointmentPage = util.getAppointmentPageById(appointmentId, AppointmentTypeEnum.REPAIR.name());
//            CommonUtil.checkResult("预约状态", AppointmentConfirmStatusEnum.AGREE.getStatusName(), newAppointmentPage.getAppointmentStatusName());
//            CommonUtil.checkResult("是否可确认", false, newAppointmentPage.getIsCanConfirm());
//            CommonUtil.checkResult("是否可接待", true, newAppointmentPage.getIsCanReception());
//            CommonUtil.checkResult("是否可取消", false, newAppointmentPage.getIsCanCancel());
...
//            CommonUtil.checkResult("跟进后跟进备注", EnumDesc.DESC_BETWEEN_40_50.getDesc(), followEvaluatePage.get(0).getFollowUpRemark());
    ...
    public void appointmentManager_testDriver() {
        ...
        try {
            ...
            Date date = DateTimeUtil.addDay(new Date(), i);
            visitor.login(APPLET_USER_ONE.getToken());
            Long appointmentId = util.appointment(AppointmentTypeEnum.TEST_DRIVE, DateTimeUtil.addDayFormat(new Date(), i));
            int appointmentNumber = util.appointmentNumber(date, AppointmentTypeEnum.TEST_DRIVE.name());

...
public class CommonUsedUtil {
    ...
    public Long getRuleAdd(String type) {
        ...
        rule.put("type", type);
        if (type.equals(RuleEnum.BLACK_LIST.getType())) {
            //新建风控规则
            IScene scene = AddScene.builder()
                    ...
                    .build();
            id = visitor.invokeApi(scene).getLong("id");
        } else {
            //新建风控规则
            IScene scene = AddScene.builder()
                    .name(pp.observeName)
    ...
    public String createAccountNumber(String name, String phone) {
        ...
        object.put("shop_id", "54172");
        object.put("shop_name", "AI-Test(门店订单录像)");
        shopList.add(shopObject);
        role.put("shop_list",shopList);
        role.put("role_id","4945");
        role.put("role_name","超级管理员");
        JSONObject body= new JSONObject();
        body.put("role_list",roleList);
        body.put("name",name);
...
//                        "        \"openid\": \"823849023iidijdiwiodede3330\",\n" +
    ...
    public String getPicPath() {
        String path = "src/main/java/com/haisheng/framework/testng/bigScreen/fengkongdaily/util/pic/yaqx.png";
        String picture = new ImageUtil().getImageBinary(path);
        IScene scene = FileUpload.builder().isPermanent(false).permanentPicType(0).pic(picture).ratio(1.50).build();
     ...
     * 角色的ID与名字的转换
     */
    public String idTransName(Long roleId) {
        String name = "";

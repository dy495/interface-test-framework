...
public class SystemCase extends TestCaseCommon implements TestCaseStd {
    ...
    public void newPotentialCustomerErr() {
        ...
        try {
            ...
            //不填写所属销售 bug 9609
            int code6 = PreSaleCustomerCreatePotentialCustomerScene.builder().customerName(name).customerPhone(phone).customerType(type).sex(sex).carStyleId(car_style_id).carModelId(car_model_id).shopId(shop_id).build().visitor(visitor).getResponse().getCode();
    ...
    public void cstmEdit() {
        ...
        try {
            ...
            String type = "PERSON";
            int sex = 1;
            Long car_style_id = PreSaleCustomerStyleListScene.builder().shopId(info.oneshopid).build().visitor(visitor).execute().getJSONArray("list").getJSONObject(1).getLong("style_id");
            Long car_model_id = PreSaleCustomerModelListScene.builder().styleId(car_style_id).build().visitor(visitor).execute().getJSONArray("list").getJSONObject(0).getLong("model_id");
            Response obj = PreSaleCustomerEditScene.builder().customerId(custID).customerName(name).customerPhone(phone).subjectType(type).sex(sex).carStyleId(car_style_id).intentionCarModelId(car_model_id).shopId(info.oneshopid).build().visitor(visitor).getResponse();
    ...
    public void newCstmRecord(String name, String phone, String type, String sex, String mess, String chk) {
        ...
        try {
            ...
            if (chk.equals("false")) {
                ...
                int total = preSaleCustomerPageScene.visitor(visitor).execute().getInteger("total");

                Response response = PreSaleCustomerCreateCustomerScene.builder().customerName(name).customerPhone(phone).customerType(type).sex(sex).carStyleId(car_style_id).carModelId(car_model_id).salesId(salesId).shopId(shop_id).vehicleChassisCode("ASDFUGGDSF02" + (int) ((Math.random() * 9 + 1) * 10000)).purchaseCarDate(dt.getHistoryDate(-1)).build().visitor(visitor).getResponse();
                Preconditions.checkArgument(response.getCode() == 1000, mess + "期待创建成功，实际" + response.getCode());

                int oneTotal = preSaleCustomerPageScene.visitor(visitor).execute().getInteger("total");
...
//                Preconditions.checkArgument(object2.getString("customer_phone").equals(phone), "创建潜客户的成交记录，客户联系方式变为：" + object2.getString("customer_phone"));
                ...
                int secondTotal = preSaleCustomerPageScene.visitor(visitor).execute().getInteger("total");
...
//                Preconditions.checkArgument(object4.getString("customer_phone").equals(phone), "创建成交客户的成交记录，客户联系方式变为：" + object2.getString("customer_phone"));
                ...
                int thirdTotal = preSaleCustomerPageScene.visitor(visitor).execute().getInteger("total");

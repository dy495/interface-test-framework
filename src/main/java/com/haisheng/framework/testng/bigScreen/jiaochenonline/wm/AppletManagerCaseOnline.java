...
public class AppletManagerCaseOnline extends TestCaseCommon implements TestCaseStd {
    ...
    public void appointmentManager_maintain() {
        try {
            ...
            int appointmentPageTotal = appointmentPageScene.visitor(visitor).execute().getInteger("total");
            ...
            int newAppointmentPageTotal = appointmentPageScene.visitor(visitor).execute().getInteger("total");
            ...
            AppAppointmentHandleScene.builder().id(appointmentId).shopId(shopId).type(10).build().visitor(visitor).execute();
            ...
            int pcReceptionPageNum = receptionPageScene.visitor(visitor).execute().getInteger("total");
            AppAppointmentReceptionScene.builder().id(appointmentId).build().visitor(visitor).execute();
            ...
            util.loginPc(ACCOUNT);
            int newPcReceptionPageNum = receptionPageScene.visitor(visitor).execute().getInteger("total");
            ...
            util.loginPc(ACCOUNT);
            ReceptorChangeScene.builder().id(receptionId).receptorId(util.getReceptorList(ACCOUNT).getUid()).shopId(shopId).build().visitor(visitor).execute();
            //登录此人app完成接待
            util.loginApp(ACCOUNT);
            int finishReceptionNum = util.getReceptionPageNum();
            AppReceptionFinishReceptionScene.builder().id(receptionId).shopId(shopId).build().visitor(visitor).execute();
            ...
            JSONObject object = appletAppointmentListScene.visitor(visitor).execute().getJSONArray("list").getJSONObject(0);
            ...
            JSONObject messageObject = appletMessageListScene.visitor(visitor).execute().getJSONArray("list").getJSONObject(0);
            ...
            JSONObject messageDetailObject = appletMessageDetailScene.visitor(visitor).execute();
            ...
            Long total = evaluatePageScene.visitor(visitor).execute().getLong("total");
            util.loginApplet(APPLET_USER_ONE);
            AppletEvaluateSubmitScene.builder().id(receptionId).shopId(shopId).type(1).score(4).isAnonymous(true).describe(EnumDesc.DESC_BETWEEN_40_50.getDesc()).suggestion(EnumDesc.DESC_BETWEEN_40_50.getDesc()).build().visitor(visitor).execute();
            //评价完成后
            JSONObject newMessageDetailObject = appletMessageDetailScene.visitor(visitor).execute();
            ...
            String newStatusName = appletAppointmentListScene.visitor(visitor).execute().getJSONArray("list").getJSONObject(0).getString("status_name");
            CommonUtil.checkResult("完成评价后applet预约状态", MaintainStatusEnum.EVALUATED.getStatusName(), newStatusName);
            util.loginPc(ACCOUNT);
            JSONObject evaluatePageObject = evaluatePageScene.visitor(visitor).execute();
            ...
            AppFollowUpCompleteScene.builder().id(followId).shopId(shopId).remark(EnumDesc.DESC_BETWEEN_40_50.getDesc()).build().visitor(visitor).execute();
            util.loginPc(ACCOUNT);
    ...
    public void appointmentManager_testDriver() {
        try {
            ...
            CommonUtil.checkResult("变更接待后接待人员", receptorList.getName(), newReceptionPage.getReceptionSaleName());
            util.loginPc(ACCOUNT);
            AppReceptorChangeScene.builder().id(receptionId).receptorId(util.getReceptorList(ACCOUNT).getUid()).shopId(shopId).build().visitor(visitor).execute();
            //登录此人app完成接待
            util.loginApp(ACCOUNT);
            int finishReceptionNum = util.getPreSalesReceptionPageNum();
            util.editCustomerInfo();
            AppFinishReceptionScene.builder().id(receptionId).shopId(shopId).build().visitor(visitor).execute();
            ...
            Long total = evaluatePageScene.visitor(visitor).execute().getLong("total");
            util.loginApplet(APPLET_USER_ONE);
            AppletEvaluateSubmitScene.builder().id(receptionId).shopId(shopId).type(4).score(4).isAnonymous(true).describe(EnumDesc.DESC_BETWEEN_40_50.getDesc()).suggestion(EnumDesc.DESC_BETWEEN_40_50.getDesc()).build().visitor(visitor).execute();
            //评价完成后
            JSONObject newMessageDetailObject = appletMessageDetailScene.visitor(visitor).execute();
            Boolean newIsCanEvaluate = newMessageDetailObject.getBoolean("is_can_evaluate");
            CommonUtil.checkResult("完成评价后applet消息中可评价状态", false, newIsCanEvaluate);
    ...
    public void integralMall_data_1() {
        try {
            ...
            Integer integral = homePageScene.visitor(visitor).execute().getInteger("integral");
    ...
    public void integralMall_data_2() {
        try {
            ...
            util.loginPc(ACCOUNT);
            int total = ExchangePageScene.builder().status(IntegralExchangeStatusEnum.WORKING.name()).build().visitor(visitor).execute().getInteger("total");
    ...
    public void integralMall_system_3() {
        try {
            ...
            if (a == null) {
                ...
                EditExchangeStockScene.builder().id(exchangePage.getId()).changeStockType(ChangeStockTypeEnum.MINUS.name()).num("1").goodsName(exchangePage.getGoodsName()).type(exchangePage.getExchangeType()).build().visitor(visitor).execute();
            } else {
                exchangePage = a;
            }
            util.loginApplet(APPLET_USER_ONE);
        ...
        try {
            ...
            if (a == null) {
                ...
                if (b == null) {
                    ...
                    EditExchangeStockScene.builder().changeStockType(ChangeStockTypeEnum.ADD.name()).num("1").id(exchangePage.getId()).goodsName(exchangePage.getGoodsName()).type(CommodityTypeEnum.FICTITIOUS.name()).build().visitor(visitor).execute();
                }
            } else {
                //如果有直接使用
                exchangePage = a;
            ...
            //兑换积分
            Long specificationId = AppletCommodityDetailScene.builder().id(exchangePage.getId()).build().visitor(visitor).execute().getLong("id");
            AppletIntegralExchangeScene.builder().id(specificationId).build().visitor(visitor).execute();
    ...
    public void integralCenter_system_5() {
        try {
            ...
            util.loginApplet(APPLET_USER_ONE);
            JSONObject response = AppletSignInDetailScene.builder().build().visitor(visitor).execute();
            ...
            //签到
            AppletSignInScene.builder().build().visitor(visitor).execute();
            //签到完成后数据校验
            JSONObject newResponse = AppletSignInDetailScene.builder().build().visitor(visitor).execute();

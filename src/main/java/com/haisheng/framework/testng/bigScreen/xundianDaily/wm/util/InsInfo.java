...
public class InsInfo {
     ...
     */
    public InsInfo(VisitorProxy visitor) {
...
//    public final List<String> nxauth_list = Arrays.asList(nxauth_name);
    ...
    public JSONObject newFirstCategory(String name) {
        ...
        String logo = FileUpload.builder().permanentPicType(0).ratioStr("1:1").pic(new ImageUtil().getImageBinary(filePath)).build().invoke(visitor, true).getString("pic_path");
        int code = CreateCategoryScene.builder().categoryLevel("FIRST_CATEGORY").belongPic(logo).categoryName(name).build().invoke(visitor, false).getInteger("code");
        Long id = CategoryPageScene.builder().build().invoke(visitor, true).getJSONArray("list").getJSONObject(0).getLong("id");
        obj.put("code", code);
        obj.put("id", id);
        return obj;
    }
